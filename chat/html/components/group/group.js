  ...
  module.exports = Vue.extend({
      ...
      data: function() {
          ...
          return {
          	group:gro,
          	gridColumns:[
               {name:'name',displayName:'分组名称'}, 
               {name:'pid',displayName:'上级分组',render:'xxx'}, 
      ...
      methods:{
          search:function (){
              $("#groupRoot form#query-form input#pageNo").val("1");
          	this.$refs.grid.query();
          },goAdd:function(){
          	routers.route.go({ path: '/group-input/'+'-1', params: { id: '-1' }}) 
  		},goEdit:function(v){
  			...
  			if(null != nid){
  				...
  				routers.route.go({ path: '/group-input/'+nid, params: { id: nid }})
              }
  					...
  					vueUI.ajaxCall({
  						...
  						success:function (res){
  							if(res.meta.success){
  								...
  								vueUI.alert("错误提示",res.meta.message)
  							}
          	...
          	for (var i = 0; i < self.$data.group.length; i++) {
  				if(self.$data.group[i].id==x.pid)
  					return self.$data.group[i].name;
  			}
          	return '';
          }
      }
